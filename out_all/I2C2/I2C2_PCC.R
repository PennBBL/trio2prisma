...
# Find all images
imgpath <- list.files(pattern="*_ds.nii.gz",recursive=TRUE)
...
#system.time( pcc.lambda <- I2C2(omnimat, id = bblids_input, visit = visit_input, J = 4, I = 10, p = numelem) )
system.time( pcc.lambda <- I2C2(omnimat, id = bblids_input, visit = visit_input, J = 2, I = 10, p = numelem, demean = TRUE) )
 
pcc.lambda$lambda
 
#### Computing the 95% CI of I2C2
system.time( pcc.ci <- I2C2.mcCI( omnimat, id = bblids_input, visit = visit_input, J = 2, I = 10, p = numelem, R = 100, rseed = 99, demean = TRUE, ci = 0.95 ) )
x11()
hist( unlist(pcc.ci) )
 
#### Compute the Null Distribution of I2C2
#### For input, demeaned data were used.
system.time( pcc.NullDist <- I2C2.mcNulldist( pcc.lambda$demean_y, id = bblids_input, visit = visit_input, J = 2, I = 10, R = 100, rseed = 1, demean = FALSE ) )  
x11()
hist( unlist(pcc.NullDist), main = 'NullDistribution' )
 
#### Draw the beanplot
library(beanplot)
beanplot( data.frame(PCC = pcc.NullDist), border = 8, ylim = c(-0.5,1), cex.main = 2, cex.axis = 1.5, main = "I2C2-PCC seed maps", ll = 0.001, col = c(8, 8, "#B2DF8A") )
lines( rep(1, 2), pcc.ci$CI, col = 1, lwd = 3, lty = 1)
lines( c(0.8, 1.2), rep(pcc.ci$CI[1], 2), col = 1, lty = 1, lwd = 3)
lines( c(0.8, 1.2), rep(pcc.ci$CI[2], 2), col = 1, lty = 1, lwd = 3)
lines(c(0.9, 1.1), rep(pcc.lambda$lambda,2), col = 2, lwd = 4, lty = 1)
